...
This is a Svelte 5/SvelteKit application that analyzes KML files of aircraft flight paths to detect circular patterns (Targets of Interest - TOI). The application extracts information about detected circles and displays them on an interactive map using MapLibre GL.
...
# Testing
pnpm run test          # Run all tests (unit and e2e)
...
1. **KML Analysis Engine** (`src/lib/kmlCircleAnalyzer.ts`)
   - Parses KML files and extracts flight path coordinates
   - Implements circle detection algorithm to identify circular patterns
   - Returns analysis results with detected circles and their properties

2. **UI Components** (in `src/lib/`)
   - `FileDropZone.svelte`: Handles file drag-and-drop for KML upload
   - `MapView.svelte`: Renders flight paths and detected circles on interactive map
   - `ToiList.svelte`: Displays list of detected TOIs with their properties

3. **Main Application** (`src/routes/+page.svelte`)
...
1. User drops KML file into FileDropZone
2. File content is passed to `analyzeKMLText()` function
3. Analysis results containing circles and flight paths are stored in component state
4. MapView renders the geographic data
5. ToiList displays detected circles
6. User can click TOIs in list to focus map view

## Important Notes

- This application uses Svelte 5's new runes API (`$state`, `$props`, etc.)
- All components should follow Svelte 5 syntax conventions
- The application is designed for defensive security analysis purposes
- Test configuration supports both browser-based component tests and Node.js unit tests

# toi-analyzer

This project is a svelte/sveltekit 5 app that analyzes kml files of aircraft
flight paths and detects circles. It extracts information about the circles and

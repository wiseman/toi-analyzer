...
export function parseKML(kmlText) {
	...
	const coords = [];
	const trackCoordMatches = [
		...kmlText.matchAll(
			/<gx:coord>\s*([\-0-9\.]+)\s+([\-0-9\.]+)(?:\s+([\-0-9\.]+))?\s*<\/gx:coord>/g
		)
	];
	const whenMatches = [...kmlText.matchAll(/<when>\s*([^<]+)\s*<\/when>/g)]
		.map((m) => Date.parse(m[1]))
		.filter((t) => !Number.isNaN(t));
	if (trackCoordMatches.length > 0) {
		const numPairs = Math.min(
			trackCoordMatches.length,
			whenMatches.length || trackCoordMatches.length
		);
		for (let i = 0; i < numPairs; i++) {
...
export function mergeCircles(circles) {
	...
	for (let i = 0; i < circles.length; i++) {
		...
		for (let j = i + 1; j < circles.length; j++) {
			...
			if (dist < Math.max(circles[i].radius, circles[j].radius) * 0.5) {
		...
		if (group.length > 1) {
			...
			const bestQuality = group.reduce((best, c) =>
				c.variance < best.variance ? c : best
			).quality;
			...
			merged.push({
				...
				contributors: group.map((c) => ({ center: c.center, radius: c.radius })),
				startIndex,
				endIndex
			});
		} else {
			merged.push({
				...group[0],
				passes: 1,
				contributors: [{ center: group[0].center, radius: group[0].radius }],
...
export function analyzeCirclesMulti(points, windowSizes = [20, 40, 80, 160, 320, 640, 1280]) {
	...
	return merged.map((c) => {
		...
		const totalOnStationMs = sessions.reduce(
			(s, sess) => s + (Number.isFinite(sess.durationMs) ? sess.durationMs : 0),
			0
		);
		return {
			...c,
			orbits: estimateOrbitCount(points, c.center, c.radius),
			startMs,
			endMs,
			durationMs,
			sessions,
...
export function circlePolygon(center, radiusMeters, steps = 128) {
	...
	for (let i = 0; i <= steps; i++) {
		...
		const lat2 = Math.asin(
			Math.sin(lat) * Math.cos(angDist) + Math.cos(lat) * Math.sin(angDist) * Math.cos(bearing)
		);
		const lon2 =
			lon +
			Math.atan2(
				Math.sin(bearing) * Math.sin(angDist) * Math.cos(lat),
...
export function computeOnStationSessions(points, circle) {
	...
	const endIdxExclusive = Math.max(
		startIdx + 1,
		Math.min(points.length, circle.endIndex ?? points.length)
	);
	...
	for (let i = 0; i < sessions.length; i++) {
		...
		if (
			Number.isFinite(prev.endMs) &&
			Number.isFinite(curr.startMs) &&
			curr.startMs - prev.endMs < thresholdMs
		) {
...
export function computeOnStationArcs(points, circles) {
	...
	for (let idx = 0; idx < circles.length; idx++) {
		...
		const endIdxExclusive = Math.max(
			startIdx + 1,
			Math.min(points.length, c.endIndex ?? points.length)
		);
		...
		for (let i = startIdx; i < endIdxExclusive; i++) {
			...
			if (r >= lower && r <= upper) {
				...
				if (currentAngles.length >= 2) {
					...
					arcFeature.properties.endMs =
						points[Math.max(startIdx, Math.min(points.length - 1, i - 1))]?.timeMs;
		...
		if (currentAngles.length >= 2) {
			...
			arcFeature.properties.startMs = points[startIdx]?.timeMs;
			arcFeature.properties.endMs =
				points[Math.max(startIdx, Math.min(points.length - 1, endIdxExclusive - 1))]?.timeMs;
			features.push(arcFeature);
		}
	}
	...
	for (let idx = 0; idx < circles.length; idx++) {
		...
		for (let i = startIdx; i < endIdxExclusive; i++) {
			...
			if (on) {
				...
				const startMs = Number.isFinite(points[runStartIdx]?.timeMs)
					? points[runStartIdx].timeMs
					: undefined;
		...
		if (run.length >= 2) {
			const startMs = Number.isFinite(points[runStartIdx]?.timeMs)
				? points[runStartIdx].timeMs
				: undefined;
			const endMs = Number.isFinite(points[endIdxExclusive - 1]?.timeMs)
				? points[endIdxExclusive - 1].timeMs
				: undefined;
			features.push({
				type: 'Feature',
				properties: { toiId: idx, quality: c.quality, startMs, endMs },
...
export function analyzeKMLText(kmlText, windowSizes = [20, 40, 80, 160, 320, 640, 1280]) {
	...
	return {
		...
		toiCentersGeoJSON,
		toiCirclesGeoJSON,
		toiOnStationArcsGeoJSON,
		flightOnStationGeoJSON
	};
}
